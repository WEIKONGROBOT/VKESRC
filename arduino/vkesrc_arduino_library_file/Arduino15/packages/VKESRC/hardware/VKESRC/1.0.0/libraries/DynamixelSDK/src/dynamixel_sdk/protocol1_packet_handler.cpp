...
int Protocol1PacketHandler::syncReadTx(PortHandler *port, uint16_t start_address, uint16_t data_length, uint8_t *param, uint16_t param_length)
{
  ...
  // 8: HEADER0 HEADER1  ID LEN INST START_ADDR DATA_LEN CRC8

  if (txpacket == NULL)
    return result;
  
  txpacket[PKT_ID]            = BROADCAST_ID;
  txpacket[PKT_LENGTH]      = DXL_LOBYTE(param_length + 4); // 4: INST START_ADDR DATA_LEN CRC8
  txpacket[PKT_INSTRUCTION]   = INST_SYNC_READ;
  txpacket[PKT_PARAMETER0+0]  = DXL_LOBYTE(start_address);
  txpacket[PKT_PARAMETER0+1]  = DXL_LOBYTE(data_length);
  ...
  result = txPacket(port, txpacket);
  if (result == COMM_SUCCESS)
    port->setPacketTimeout((uint16_t)((6 + data_length) * param_length));
